...
# Standard Scaler to use with dictionares {data, biocovariates}
...
# Limitations:
# Not compatible with (Random/Grid)SearchCV from keras
# 'cv method' option attribute not finalized
# It can harmonize global measures or other features-of-no-interest, like Total Intracravial Volumes,
# by including them in feat_of_no_interest, BUT ONLY using the fit_transform method.
# Otherwise, using fit() and transform() separatly, it harmonized 2xtimes.
...
class ComBatHarmonization(BaseEstimator, TransformerMixin):
  ...
  def __init__(self, cv_method, ref_batch, regression_fit, feat_detail, feat_of_no_interest):
    ...
    self.cv_method=cv_method # Attribute that defines the CV method, for now should be set to None always
    self.ref_batch=ref_batch # if there is a reference set M-ComBat is used
    self.regression_fit=regression_fit # will data be used after for biological covariates correction? True or False
    self.feat_detail=feat_detail # a dictionary: details of features-of-interest to harmonize
    self.feat_of_no_interest=feat_of_no_interest # (optional) reccomendation is to not use this option: a dictionary: details of features-of-no-interest to harmonize


  def extract_data(self,X):
    """
    Function is called to extract data.
    ...
    elif self.cv_method: # This option has not been developed yet: cv_method should be =None
      X_data=X.copy()
...
"""
In development : Include option of estimating Beta coefficients but then don't correct for it.
